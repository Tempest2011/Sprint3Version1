<div class="metodologias index">
	<h2><?php echo __('Metodologias'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('nombre'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	<?php foreach ($metodologias as $metodologia): ?>
	<tr>
		<td><?php echo h($metodologia['Metodologia']['id']); ?>&nbsp;</td>
		<td><?php echo h($metodologia['Metodologia']['nombre']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $metodologia['Metodologia']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $metodologia['Metodologia']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $metodologia['Metodologia']['id']), null, __('Are you sure you want to delete # %s?', $metodologia['Metodologia']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Metodologia'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Metienerols'), array('controller' => 'metienerols', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Metienerol'), array('controller' => 'metienerols', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Metodologiaempresas'), array('controller' => 'metodologiaempresas', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Metodologiaempresa'), array('controller' => 'metodologiaempresas', 'action' => 'add')); ?> </li>
	</ul>
</div>
